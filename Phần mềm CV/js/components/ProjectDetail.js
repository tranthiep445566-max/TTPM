...
import {
  ...
  listProjectTechnologies, setProjectTechnologies, listInstallments, addInstallment, updateInstallment,
...
export default {
  ...
  setup() {
    ...
    const newInstallment = reactive({ name: '', percent: null, amount: 0, due_note: '' });
    ...
    async function submitInstallment() {
      ...
      try {
        await addInstallment({
          ...
          amount: newInstallment.amount, due_note: newInstallment.due_note, sort_order: installments.value.length
        });
        Object.assign(newInstallment, { name: '', percent: null, amount: 0, due_note: '' });
    ...
    async function saveDueNote(inst) {
      try {
        await updateInstallment(inst.id, { due_note: inst.due_note });
      } catch (e) { toast(e.message, 'error'); }
    }
    ...
    return {
      ...
      submitInstallment, removeInstallment, saveDueNote, openPay, askConfirmPay, doConfirmPay, saveTimeline, autoWarrantyEnd,
  ...
  template: `
    <div class="page" v-if="!loading && project">
      <div class="page-header">
        ...
        <div>
          <a class="btn btn-sm" :href="'#/projects/' + project.id + '/quote-print'">🧾 Xuất báo giá</a>
          <a class="btn btn-sm" :href="'#/projects/' + project.id + '/contract-print'">📄 Xuất hợp đồng</a>
      ...
      <div v-show="activeTab === 'finance'" class="detail-card">
        ...
        <div class="form-group" v-if="can('projects','can_edit')">
          <input class="input" placeholder="Mô tả thời điểm thanh toán (dùng trong hợp đồng, VD: Đặt cọc ngay khi ký hợp đồng)" v-model="newInstallment.due_note" />
        </div>
        <table class="table">
          <thead><tr><th>Tên đợt</th><th>%</th><th>Số tiền</th><th>Thời điểm thanh toán (cho hợp đồng)</th><th>Trạng thái</th><th>Ngày TT</th><th></th></tr></thead>
          <tbody>
            <tr v-for="i in installments" :key="i.id">
              <td>{{ i.name }}</td><td>{{ i.percent ? i.percent + '%' : '—' }}</td><td>{{ formatCurrency(i.amount) }}</td>
              <td>
                <input v-if="can('projects','can_edit')" class="input-sm" style="width:220px" v-model="i.due_note" @change="saveDueNote(i)" />
                <span v-else>{{ i.due_note || '—' }}</span>
              </td>
