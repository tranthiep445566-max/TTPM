...
function emptyForm() {
  return {
    id: null, company_name: '', contact_person: '', phone: '', email: '', industry: '', note: '',
    address: '', tax_code: '', representative_name: '', representative_position: ''
  };
}
    ...
    async function save() {
      try {
        const payload = {
          ...
          email: form.email, industry: form.industry, note: form.note, address: form.address,
          tax_code: form.tax_code, representative_name: form.representative_name,
  ...
  template: `
    <div class="page">
      ...
      <Modal v-model="modalOpen" :title="editing ? 'Sửa khách hàng' : 'Thêm khách hàng'">
        ...
        <div class="form-group"><label>Địa chỉ (dùng khi xuất hợp đồng)</label><input class="input" v-model="form.address" /></div>
        <div class="form-row">
          <div class="form-group"><label>Mã số thuế</label><input class="input" v-model="form.tax_code" /></div>
          <div class="form-group"><label>Người đại diện</label><input class="input" v-model="form.representative_name" /></div>
          <div class="form-group"><label>Chức vụ đại diện</label><input class="input" v-model="form.representative_position" /></div>
        </div>
